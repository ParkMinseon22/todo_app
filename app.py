...
class Todo:
    ...
    # 객체가 리스트 안에 있을 때 리스트 안의 요소들을 출력하면 __repr만 나온다. (__str__은 안나옴)
    def __repr__(self):
        return f'Task: {self.__task}, Done: {self.__done}'
...
# todo = Todo('숙제하기')
# print(id(todo))
# print(Todo(task='쁘띠 밥주기', done=True))
...
st.text_input('새로운 할일 추가',key='new_task', on_change=add_todo) # input 창에 내용을 작성(기존과 다른 내용)하고 엔터하면 add_todo 함수 호출
